...
describe Program do
  ...
  describe "should validate the uniqueness of channel for a specified region, title, start_datetime, and end_datetime" do
   ...
   context "where the title is different" do
      it "creates a program" do
        FactoryGirl.build(:valid_program, channel_id: @channel_nine.id, region_id: @region_melbourne.id, sport_id: @sport_cricket.id, title: "Some other title").valid?.should be_true
      end
    end
